...
problem = {
    ...
    "startA": ["space on table", "space on b", "b on table", "space on c", "c on a", "a on table"],
    "finishA": ["b on table", "space on b", "a on table", "space on a", "c on table", "space on c"],

    "startB": ["a on table", "b on table", "c on table", "space on a", "space on b", "space on c"],
    "finishB": ["c on table", "b on c", "a on b", "space on a"],
...
def main():
    ...
    #     print(action)
    startA = problem['startA']
    finishA = problem['finishA']
    ops = problem['ops']
    actionSequenceA = gps(startA, finishA, ops)
    if actionSequenceA is None:
        print("plan failure...")
        return
    for actionA in actionSequenceA:
        print(actionA)

    print("\n\n")

    startB = problem['startB']
    finishB = problem['finishB']
    ops = problem['ops']
    actionSequenceB = gps(startB, finishB, ops)
    if actionSequenceB is None:
        print("plan failure...")
        return
    for actionB in actionSequenceB:
        print(actionB)
